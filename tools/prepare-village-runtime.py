...
for a in manifest['assets']:
    if 'MAKKO_ANIM.'+a['key']+'=' not in text:
        meta = {k:a[k] for k in ('frames','fw','fh','anchorX')}
        text += 'MAKKO_ANIM.'+a['key']+'='+json.dumps(meta)+';\n'
...
animations={role+'_'+action:'./media/anim/'+role+'_'+action+'.png' for role in sources for action in ('idle','walk','run')}
...
keys = [role+'_'+action for role in sources for action in ('idle','walk','run')]
for page in range(0,len(keys),6):
    ...
    canvas.save(root/f'reports/ratkin-art/village-{page//6+1}.jpg',quality=92)
